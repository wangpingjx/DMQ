...
class SockerWorker {
     ...
     * @var string
     */
    protected $eventLoopName ="\\DMQ\\Lib\\Events\\Select";
    ...
    {
        $this->eventLoopName = "\\DMQ\\Lib\\Events\\".$event_loop_name;
        require_once DMQ_ROOT_DIR . 'events/'.ucfirst(str_replace('DMQ', '', $event_loop_name)).'.php';
    }
    ...
    public function signalHandler($signal, $null = null, $null = null)
    {
        switch($signal)
        {
            ...
            case SIGHUP:
                ...
                if(\DMQ\Lib\Config::get($this->workerName.'.no_reload'))
                {
                    return;
                }
                $this->stop();
